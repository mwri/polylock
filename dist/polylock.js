...
let polylock = (function() {
	...
	let polylock = function polylock (params) {
	...
	polylock.prototype.exec = function (op_fun, locks) {
	...
	polylock.prototype.test_locks = function (req_locks) {
		...
		for (let i = 0; i < numof_req_resources; i++) {
			...
			if (req_resource in this.ex_locks && this.ex_locks[req_resource] > 0)
				ex_lock = 2;
			if (req_resource in this.ex_reserved && this.ex_reserved[req_resource] > 0)
				reserved = 1;
			if (req_resource in this.sh_locks && this.sh_locks[req_resource] > 0)
	...
	polylock.prototype.take_locks = function (req_locks) {
		...
		for (let i = 0; i < numof_req_resources; i++) {
			let req_resource = req_resources[i];
			if (req_locks[req_resource] === 'write') 
				this.ex_locks[req_resource] = req_resource in this.ex_locks
					? this.ex_locks[req_resource] + 1
					: 1;
			else
				this.sh_locks[req_resource] = req_resource in this.sh_locks
					? this.sh_locks[req_resource] + 1
					: 1;
		}
	...
	polylock.prototype.reserve_locks = function (req_locks) {
		...
		for (let i = 0; i < numof_req_resources; i++) {
			let req_resource = req_resources[i]; 
			if (req_locks[req_resource] === 'write') 
				this.ex_reserved[req_resource] = req_resource in this.ex_reserved
					? this.ex_reserved[req_resource] + 1
					: 1;
		}
	...
	polylock.prototype.yield_locks = function (red_locks) {
		...
		for (let i = 0; i < numof_red_resources; i++) {
			...
			if (red_locks[red_resource] === 'write')
				this.ex_locks[red_resource]--;
			else
				this.sh_locks[red_resource]--;
		}
	...
	polylock.prototype.run_queue = function () {
		...
		for (let node = this.queue.head; node !== undefined; node = node.next) {
	...
	let polylock_ll = function polylock_ll () {
